...
class SocialController extends Controller
{
    ...
    public function getSocialCallback($provider, Request $request)
    {
        if ($request->exists('error_code')) {
            return Redirect::route('login')->withErrors(['msg' => $provider . '登入或綁定失敗，請重新再試']);
        }
        ...
        if (!empty($s_u)) {
            ...
            if (empty($socialite_user->email)) {
                return Redirect::route('login')->withErrors(['msg' => '無法從' . $provider . '獲取信箱資料，請由其他方式進行註冊']);
            }
            ...
            if (!empty($user)) {
                ...
                if (!empty($s_user)) {
                    return Redirect::route('login')->withErrors(['msg' => 'Email已被綁定，請由其他方式登入']);
                } else {
