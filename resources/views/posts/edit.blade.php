                    ...
                    <div class="container-fluid">
                        <form action="{{ route('posts.update',  ['id'=>$post->id]) }}" method="POST" class="mt-2">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <select name="type" id="type" class="form-control form-control-md">
                                        ...
                                        <option value="{{ $post_type->id }}" "{{ ($post->type == $post_type->id ) ? "selected" : "" }}">{{ $post_type->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <textarea rows="15" class="form-control form-control-md" name="content" id="content" style="resize:vertical;">{{ $post->content ?? '' }}</textarea>
                                </div>
                            </div>
                            <div class="form-group row">
                                <div class="col-md-10 offset-md-2">
                                    <button class="btn btn-md btn-primary" type="submit">儲存</button>
                                </div>
                            </div>
