                        ...
                        <div class="col-md-4">
                            ...
                            <div class="float-right ml-auto">
                                <form action="{{ route('posts.comments.destroy', ['post'=>$post->id, 'comment'=>$comment->id]) }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="_method" value="DELETE">
                                    <button type="submit" class="btn btn-md btn-danger"><i class="fas fa-trash"></i><span class="pl-1">刪除回應</span></button>
                                </form>
                            </div>
                            @endif
                            @endauth
                    ...
                    <form action="{{ route('posts.comments.store', ['post' => $post->id]) }}" method="POST">
                        ...
                        <div class="form-group row mb-0">
                            <div class="col-md-8 offset-md-2">
                                <button type="submit" class="btn btn-primary btn-block">發表</button>
                            </div>
                        </div>
                    </form>
                    @else
